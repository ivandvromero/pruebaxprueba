...
export class DatabaseService implements OnModuleDestroy, OnApplicationShutdown {
  ...
  public static removeOrphanConnections = false;
  ...
  async removeOldConnections(newConnection: Connection) {
    ...
    if (removedConnections.length) {
      try {
        const closedConnections = await DatabaseService.closeConnections(
          removedConnections,
        );
        return this.logger?.log(
          `closed old database connection(s): ${closedConnections
            .map((c) => c.name)
            .join(', ')}, ${inspect({ meta })}`,
        );
      } catch (err) {

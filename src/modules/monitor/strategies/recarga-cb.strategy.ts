...
@Injectable()
export class RecargaCBCachStrategy implements ProviderStrategy {
  constructor(
  ...
  public async doAlgorithm(
    ...
    data.transactionTransform.Field_K7_0091 = 'Recarga CB';
    data.transactionTransform.Field_K7_0092 =
      TypeTransactionPts.Cell2CellRecibir;
    data.transactionTransform.Field_K7_0102 = 'Recarga CB';
    data.transactionTransform.Field_K7_0104 = 'Recarga CB';
  ...
  async sendSmsNotification(eventObject: MessageEvent): Promise<void> {
    try {
      ...
      this.logger.error('Error sms notification atm recarga CB');
    }
  }
}

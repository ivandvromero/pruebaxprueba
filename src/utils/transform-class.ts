...
export const transformDate = (datetime): string => {
  const currentDate =
...
export const transformTime = (datetime): string => {
  const currentDate =
    typeof datetime === 'object' ? datetime : new Date(datetime);
...
export const formatDataProduct = (product) => {
  product.creationDate = product.CreationDate
    ...
    : 0;
  product.dl_estado_deposito_c = product.dl_EstadoDeposito_c
    ? SalesProfileStatus[product.dl_EstadoDeposito_c]
    : '5';
  return product;
};
...
export const formatDataCreationDate = (eventObject) => {
  ...
  let [result] = confirmations.sort((a, b) => (a.data.id < b.data.id ? -1 : 0));
...
export const formatDateTransactionEmpty = (baseEventLog) => {
  const mappedDetails = baseEventLog.details.map((detail) =>
    detail.key === 'date_transaction'
      ? {
          ...detail,
          value: detail.value === '' ? baseEventLog.timestamp : detail.value,
        }
      : detail,
  );

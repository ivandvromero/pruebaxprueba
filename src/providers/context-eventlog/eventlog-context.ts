...
export class ProviderEventlogContext {
  ...
  async generateEventLog(eventObject): Promise<any> {
    ...
    for (const key in result) {
      ...
      console.error(baseEventLog, 'data');
      const resultData = this.strategy.doAlgorithm(baseEventLog);
      if (
  ...
  getCellPhone(eventObject, eventCode) {
    const isReverse = this.getIsReverse(eventObject);
    return eventCode == 101 && !isReverse

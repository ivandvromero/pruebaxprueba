...
describe('ConfigurationService', () => {
  ...
  describe('getDocumentTypeFullNameById', () => {
    it('Success', async () => {
      const response = await service.getDocumentTypeFullNameById('2');
      expect(response).toBe('Cédula de ciudadanía');
    });
    it('Cache Success', async () => {
      jest
        .spyOn(cache, 'getCache')
        .mockResolvedValueOnce('Cédula de ciudadanía');
      const response = await service.getDocumentTypeFullNameById('3');
      expect(response).toBe('Cédula de ciudadanía');
    });
    it('unexpected Error', async () => {
      try {
        ...
        jest
          .spyOn(http, 'get')
          .mockReturnValueOnce(throwError(() => responseError));
        await service.getDocumentTypeFullNameById('test');
      } catch (error) {
        expect(error).toBeInstanceOf(InternalServerExceptionDale);
      }
    });

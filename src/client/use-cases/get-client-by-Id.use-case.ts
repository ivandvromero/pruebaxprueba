...
export class GetClientByIdUseCase {
  ...
  public async apply(clientId: string, hasAccount?: boolean): Promise<IClient> {
    try {
      const { client } = await this.coreConnector.getClientById(clientId);
      let account;
      if (!hasAccount) {
        account = await this.getAccountByClientIdUseCase.apply(client.clientId);
